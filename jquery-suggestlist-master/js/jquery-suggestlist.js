( function( $ ) {
	...
	function Suggestlist( element, options ) {
		...
		if ( this.isInput ) {
			this.element.on( {
				...
				'click.suggestlist': $.proxy( this.updateLi, this ),
	...
	Suggestlist.prototype = {
		...
		var val
			...
			// the blank space is also considered
			//val = $.trim( this.element.val() ).replace(/\s+/, ' '),
			...
			this.picker.find( 'li' ).each( function( i, elem ) {
				...
				var liText = $( elem ).text().toLowerCase();
				
			   // performing substring match instead of exact match
				if (liText.indexOf(val.toLowerCase())===0 && val!="") {
					if(selectFlag==0 && liText.trim()!=""){
